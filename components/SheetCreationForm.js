      ...
      {showSheetNameField ? (
        <div className="space-y-2">
          ...
          <input
            id="sheetName"
            type="text"
            placeholder="Ex: River Flows in You (Simple)"
            value={sheetName}
            onChange={(event) => onSheetNameChange(event.target.value)}
            className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2.5 text-sm text-neutral-950 ring-0 transition outline-none focus:border-neutral-950 focus:ring-1 focus:ring-neutral-950"
          />
        </div>
      ) : null}
